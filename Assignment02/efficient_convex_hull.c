...
t_line *upper_hull( t_point *points, int num_point, t_point p1, t_point pn, t_line *lines, int *num_line, int *capacity){
    ...
    for(int i = 0; i < num_point; i++){
        if(max_d < distance(a, b, c, points[i])){
            max_d = distance(a, b, c, points[i]);
            max_i = i;
        }
    }
    ...
    separate_points(points, num_point, p1, points[max_i], s1, s2, &n1, &n2);

           
    separate_points(points, num_point, points[max_i], pn, s3, s4, &n3, &n4);

    lines = upper_hull(s1, n1, p1, points[max_i], lines, num_line, capacity);
    lines = upper_hull(s3, n3, points[max_i], pn, lines, num_line, capacity);
    free(s1);
    free(s3);
    free(s4);
    free(s2);
...
void separate_points( t_point *points, int num_point, t_point from, t_point to, t_point *s1, t_point *s2, int *n1, int *n2){
    ...
    *n2 = 0;
    ...
    for(int i = 0; i < num_point; i++){
        float current_x = points[i].x;
        float current_y = points[i].y;
        
        float det = (current_x * a) + (current_y * b) - c;
        
        if((from.x == points[i].x && from.y == points[i].y) || (to.x == points[i].x && to.y == points[i].y)){
            continue;
        }
        if(det < -0.00001){
            s1[(*n1)++] = points[i];
